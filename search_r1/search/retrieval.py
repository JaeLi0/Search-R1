...
if __name__ == '__main__':
    ...
    parser.add_argument('--retrieval_query_max_length', default=256, type=int)
    ...
    parser.add_argument('--save_cache_path', type=str, default=None,
                        help='Path to save retrieval cache JSON. Format: {question: [doc dicts]}. '
                             'If not provided, auto-generates from dataset_path and data_split.')
    parser.add_argument('--max_samples', type=int, default=None,
                        help='Max number of samples to retrieve (for debugging). Default: all samples.')
    ...
    if args.max_samples is not None:
        all_split = all_split[:args.max_samples]

    questions = [sample['question'] for sample in all_split]
    print(f'Loaded {len(questions)} questions from {args.data_split} split.')

    # initialize the retriever and conduct retrieval
    retriever = get_retriever(args)
    print('Start Retrieving ...')
    results, scores = retriever.batch_search(questions, return_score=True)

    # Build retrieval cache: {question: [{"id": ..., "title": ..., "contents": ..., "text": ..., "score": ...}, ...]}
    retrieval_cache = {}
    for question, docs, doc_scores in zip(questions, results, scores):
        for doc, score in zip(docs, doc_scores):
            doc = dict(doc)  # ensure it's a plain dict
            doc['score'] = float(score)
        retrieval_cache[question] = [dict(d) for d in docs]
        for doc, score in zip(retrieval_cache[question], doc_scores):
            doc['score'] = float(score)

    # Determine save path
    if args.save_cache_path is not None:
        save_path = args.save_cache_path
    else:
        data_name = os.path.basename(os.path.normpath(args.dataset_path))
        save_dir = os.path.dirname(args.dataset_path)
        save_path = os.path.join(save_dir, f'{args.retrieval_method}_{args.data_split}_cache_top{args.retrieval_topk}.json')

    os.makedirs(os.path.dirname(save_path) or '.', exist_ok=True)
    with open(save_path, 'w', encoding='utf-8') as f:
        json.dump(retrieval_cache, f, ensure_ascii=False, indent=2)

    print(f'Retrieval cache saved to {save_path} ({len(retrieval_cache)} entries)')
